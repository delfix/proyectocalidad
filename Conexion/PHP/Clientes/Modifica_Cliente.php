...
<title>INTERCEL Modifica_Cliente</title>
            ...
            <div class="top-menu">
                ...
                try {
                    if (isset($_POST['Actualizar'])) {
                        ...
                        $datos->conexion();
                        $datos->abrir_bd();
                        $resultado = $datos->consultar("select c.*,m.m_nombre from Cliente c join municipio m on (c.c_municipio = m.m_id) where c_id = ".$id." order by c.c_nombre"); 
                        if ($resultado){
                            while($row = mysqli_fetch_array($resultado))
                            {
                ?>
                <form method="post"  style="float:center;" id="form">
                    <input type="hidden" name="id_update" value="<?php echo $row['c_id'];?>" />
                    <br><h4><p>Nombre:</p></h4>
                    <input type="text" name="nombre"id ="nombre" value="<?php echo $row['c_nombre']; ?>">
                    <br><h4><p>Apellido:</p></h4>
                    <input type="text" name="apellido" id ="apellido"value="<?php echo $row['c_apellido']; ?>">
					<br><h4><p>Nit:</p></h4>
                    <input type="text" name="nit"id ="nit" value="<?php echo $row['c_nit']; ?>">
					<br><h4><p>Direccion:</p></h4>
                    <input type="text" name="direccion"id ="direccion" value="<?php echo $row['c_direccion']; ?>">
					
					<br><h4><p>Correo:</p></h4>
                    <input type="email" name="correo" id ="correo" required="required"  value="<?php echo $row['c_correo']; ?>">
					<br><h4><p>Telefono:</p></h4>
                    <input type="text" name="telefono" id ="telefono" value="<?php echo $row['c_telefono'];?>">
                    <br>
						<script type="text/javascript">
            ...
            form.submit();
                ...
                <?php
                    ...
                    if (isset($_POST['Modificar'])) {
                        ...
                        $datos->conexion();
                        $datos->abrir_bd();
                        $datos->sql("update cliente set c_nombre='$nombre', c_apellido='$apellido', c_nit='$nit',c_direccion='$direccion', c_municipio='$municipio',c_correo='$correo',c_telefono='$telefono' where c_id='$id_update'"," ");
