...
<title>INTERCEL Modifica_Municipio</title>
            ...
            <div class="top-menu">
                ...
                try {
                    if (isset($_POST['Actualizar'])) {
                        ...
                        $datos->conexion();
                        $datos->abrir_bd();
                        $resultado = $datos->consultar("select m.*,d.d_nombre from municipio m join departamento d on (m.m_departamento = d.d_id) where m_id = ".$id." order by m.m_nombre"); 
                        if ($resultado){
                            while($row = mysqli_fetch_array($resultado))
                            {
                ?>
                <form method="post" action="Modifica_Municipio.php" style="float:center;">
                    <input type="hidden" name="id_update" value="<?php echo $row['m_id'];?>" />
                    <br><h4><p>Nombre:</p></h4>
                    <input type="text" name="nombre" value="<?php echo $row['m_nombre']; ?>">
                    <br><h4><p>Descripcion:</p></h4>
                    <input type="text" name="descripcion" value="<?php echo $row['m_descripcion'];?>">
                    <br>
                    <?php
                            }}
                        require_once "../../Conexion/Sentencias_sql.php"; 
                            ...
                            $datos->conexion();
                            $datos->abrir_bd();
                            $resultado = $datos->query("select d_id, d_nombre from departamento"); 
                            $datos->cerrar_bd();
                    ?> 
                ...
                <?php
                    ...
                    if (isset($_POST['Modificar'])) {
                        ...
                        $datos->conexion();
                        $datos->abrir_bd();
                        $datos->sql("update municipio set m_nombre='$nombre', m_descripcion='$descripcion', m_departamento='$departamento' where m_id='$id_update'"," ");
